...
async function createWidgetForm(containerId: string, buttonColor: string, backgroundColor: string, textColor: string) {
  await i18next.init({
    lng: 'en',
    debug: false,
    resources,
  ...
  lng.addEventListener('click', () => {
    i18next.language === 'en' ? i18next.changeLanguage("ru").then(() => render()) :
    i18next.changeLanguage("en").then(() => render())
    })
}
  
createWidgetForm('form-widget', '#F5A623', '#4A90E2', '#FFFFFF')
